var db = {
  "recipes": {
    "granola-bars": {
      "meta": {
        "properties": {
          "original_recipe": "https://minimalistbaker.com/healthy-5-ingredient-granola-bars/",
          "slug": "granola-bars",
          "name": "5 Ingredient Granola Bars",
          "day_made": "[2019-09-01 Sun]",
          "is_vegan": "true",
          "is_vegetarian": "false",
          "ease_of_making": "5/5",
          "rating": "5/5"
        },
        "logbook": []
      },
      "ingredients": {
        "keys": [
          "Ingredient",
          "Quantity",
          "Unit"
        ],
        "data": [
          {
            "Ingredient": "Dates (Deglet noor or medjool)",
            "Quantity": "1",
            "Unit": "cups"
          },
          {
            "Ingredient": "Maple Syrup (or: agava nectar, honey)",
            "Quantity": "1/4",
            "Unit": "cups"
          },
          {
            "Ingredient": "Natural Peanut Butter",
            "Quantity": "1/4",
            "Unit": "cups"
          },
          {
            "Ingredient": "Roasted, unsalted almonds",
            "Quantity": "1",
            "Unit": "cups"
          },
          {
            "Ingredient": "Rolled Oats",
            "Quantity": "1+1/2",
            "Unit": "cups"
          },
          {
            "Ingredient": "Optional: Chocolate chips, dried fruit, nuts etc",
            "Quantity": "-",
            "Unit": "-"
          },
          {
            "Ingredient": "Parchment Paper",
            "Quantity": "-",
            "Unit": "-"
          },
          {
            "Ingredient": "8x8 Baking Pan",
            "Quantity": "-",
            "Unit": "-"
          }
        ]
      },
      "instructions": [
        {
          "o": "Chop almonds roughly. Put them in a bowl.",
          "f": "Chop almonds roughly. Put them in a bowl."
        },
        {
          "o": "Put oats in the bowl.",
          "f": "Put oats in the bowl."
        },
        {
          "o": "Blend dates until dough-y. Put them in the bowl",
          "f": "Blend dates until dough-y. Put them in the bowl"
        },
        {
          "o": "Put maple-syrup and peanut-butter into a sausapan and heat on low. Stir to combine.",
          "f": "Put maple-syrup and peanut-butter into a sausapan and heat on low. Stir to combine."
        },
        {
          "o": "Pour mix into the bowl and stir to combine.",
          "f": "Pour mix into the bowl and stir to combine."
        },
        {
          "o": "Transfer to a baking dish (8 x 8) lined with parchment paper.",
          "f": "Transfer to a baking dish (8 x 8) lined with parchment paper."
        }
      ]
    },
    "pantry-dahl": {
      "meta": {
        "properties": {
          "original_recipe": "https://ohsheglows.com/2017/07/21/8-minute-pantry-dal-two-ways/",
          "slug": "pantry-dahl",
          "day_made": "[2019-09-01 Sun]",
          "name": "Pantry Dahl",
          "is_vegan": "true",
          "is_vegetarian": "true",
          "ease_of_making": "5/5",
          "rating": "5/5"
        },
        "logbook": []
      },
      "ingredients": {
        "keys": [
          "Ingredient",
          "Quantity",
          "Unit"
        ],
        "data": [
          {
            "Ingredient": "Coconut Oil",
            "Quantity": "1",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Diced Veggies (Carrots, Potatoes, Broccoli etc)",
            "Quantity": "4",
            "Unit": "cups"
          },
          {
            "Ingredient": "Uncooked Red Lentils",
            "Quantity": "1/2",
            "Unit": "cups"
          },
          {
            "Ingredient": "Water",
            "Quantity": "1/2+",
            "Unit": "cups"
          },
          {
            "Ingredient": "Diced Tomato",
            "Quantity": "1",
            "Unit": "cups"
          },
          {
            "Ingredient": "Coconut Milk",
            "Quantity": "1",
            "Unit": "cups"
          },
          {
            "Ingredient": "Garlic Powder",
            "Quantity": "1+1/2",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Minced Onion",
            "Quantity": "1+1/2",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "curry Powder",
            "Quantity": "1",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Sea salt",
            "Quantity": "1",
            "Unit": "tsp"
          },
          {
            "Ingredient": "Pepper",
            "Quantity": "1",
            "Unit": "pinch"
          },
          {
            "Ingredient": "Rice or Grain for Base",
            "Quantity": "-",
            "Unit": "-"
          }
        ]
      },
      "instructions": [
        {
          "o": "Melt coconut oil in a large pot",
          "f": "Melt coconut oil in a large pot"
        },
        {
          "o": "peel (if necessary) and dice veggies into 1/2 inch pieces. Add to pot and stir.",
          "f": "peel (if necessary) and dice veggies into 1/2 inch pieces. Add to pot and stir."
        },
        {
          "o": "Add the rest of the ingredients.",
          "f": "Add the rest of the ingredients."
        },
        {
          "o": "Bring to a boil and the reduce heat to medium.",
          "f": "Bring to a boil and the reduce heat to medium."
        },
        {
          "o": "[t: 00:18:00] Cook for 18-30 minutes. Stir frequently.",
          "f": "Cook for 18-30 minutes. Stir frequently.",
          "timer": "00:18:00"
        },
        {
          "o": "Serve over rice. Optional: Garnish with cilantro and lime.",
          "f": "Serve over rice. Optional: Garnish with cilantro and lime."
        }
      ]
    },
    "marinated-tofu": {
      "meta": {
        "properties": {
          "original_recipe": "https://simpleveganblog.com/easy-marinated-tofu/",
          "day_made": "[2019-09-02 Mon]",
          "slug": "marinated-tofu",
          "is_vegan": "true",
          "name": "Easy Marinated Tofu",
          "is_vegetarian": "true",
          "ease_of_making": "5/5",
          "rating": "4/5"
        },
        "logbook": []
      },
      "ingredients": {
        "keys": [
          "Ingredient",
          "Quantity",
          "Unit"
        ],
        "data": [
          {
            "Ingredient": "Tofu",
            "Quantity": "1",
            "Unit": "brick"
          },
          {
            "Ingredient": "Water",
            "Quantity": "1/4",
            "Unit": "cups"
          },
          {
            "Ingredient": "Soy Sauce",
            "Quantity": "2",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Maple syrup",
            "Quantity": "1",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Apple cider vinegar",
            "Quantity": "1",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Garlic powder",
            "Quantity": "1",
            "Unit": "tsp"
          },
          {
            "Ingredient": "",
            "Quantity": "",
            "Unit": ""
          },
          {
            "Ingredient": "",
            "Quantity": "",
            "Unit": ""
          },
          {
            "Ingredient": "",
            "Quantity": "",
            "Unit": ""
          }
        ]
      },
      "instructions": [
        {
          "o": "Press the tofu for a while. Then dice/cube it.",
          "f": "Press the tofu for a while. Then dice/cube it."
        },
        {
          "o": "Mix the marinade ingredients in a bowl",
          "f": "Mix the marinade ingredients in a bowl"
        },
        {
          "o": "Put the tofu in the bowl and cover. Put it in the fridge for 15 min.",
          "f": "Put the tofu in the bowl and cover. Put it in the fridge for 15 min."
        },
        {
          "o": "Take the tofu out and pan fry until golden brown.",
          "f": "Take the tofu out and pan fry until golden brown."
        }
      ]
    },
    "candied-ginger": {
      "meta": {
        "properties": {
          "original_recipe": "https://www.davidlebovitz.com/candied-ginger/",
          "day_made": "[2019-09-02 Mon]",
          "slug": "candied-ginger",
          "name": "Candied Ginger",
          "is_vegan": "true",
          "is_vegetarian": "true",
          "ease_of_making": "2/5",
          "rating": "4/5"
        },
        "logbook": []
      },
      "ingredients": {
        "keys": [
          "Ingredient",
          "Quantity",
          "Unit"
        ],
        "data": [
          {
            "Ingredient": "ginger",
            "Quantity": "1",
            "Unit": "lbs"
          },
          {
            "Ingredient": "sugar",
            "Quantity": "4",
            "Unit": "cups"
          },
          {
            "Ingredient": "water",
            "Quantity": "4",
            "Unit": "cups"
          },
          {
            "Ingredient": "salt",
            "Quantity": "1",
            "Unit": "pinch"
          }
        ]
      },
      "instructions": [
        {
          "o": "Peel the ginger.",
          "f": "Peel the ginger."
        },
        {
          "o": "Slice the ginger. Thin.",
          "f": "Slice the ginger. Thin."
        },
        {
          "o": "Put ginger into a pot, cover with water. Bring water to a boil.",
          "f": "Put ginger into a pot, cover with water. Bring water to a boil."
        },
        {
          "o": "[t: 00:10:00] Reduce heat and simmer for 10 minutes.",
          "f": "Reduce heat and simmer for 10 minutes.",
          "timer": "00:10:00"
        },
        {
          "o": "Repeat the previous step.",
          "f": "Repeat the previous step."
        },
        {
          "o": "Mix the sugar, water, salt and ginger slices in the pot. Cook until the temperature reaches 225ºF (106ºC). I don't have a thermometer. Some people online report cooking for 50min - 1 hour. Other recipes say \"Bring to a boil, reduce the heat to medium, and simmer uncovered for 35-40 minutes or until the temperature on a candy thermometer reads 225 degrees F.\"",
          "f": "Mix the sugar, water, salt and ginger slices in the pot. Cook until the temperature reaches 225ºF (106ºC). I don't have a thermometer. Some people online report cooking for 50min - 1 hour. Other recipes say \"Bring to a boil, reduce the heat to medium, and simmer uncovered for 35-40 minutes or until the temperature on a candy thermometer reads 225 degrees F.\""
        },
        {
          "o": "[t: 01:00:00] Remove from heat. Let stand for one hour.",
          "f": "Remove from heat. Let stand for one hour.",
          "timer": "01:00:00"
        },
        {
          "o": "Drain the ginger through a colander, catch the syrup.",
          "f": "Drain the ginger through a colander, catch the syrup."
        },
        {
          "o": "Toss drained ginger in sugar.",
          "f": "Toss drained ginger in sugar."
        },
        {
          "o": "Shake off excess sugar, and spread the ginger slices on a baking sheet or cooling rack until they’re somewhat dry.",
          "f": "Shake off excess sugar, and spread the ginger slices on a baking sheet or cooling rack until they’re somewhat dry."
        }
      ]
    },
    "shakshuka": {
      "meta": {
        "properties": {
          "original_recipe": "https://minimalistbaker.com/1-pot-chickpea-shakshuka/",
          "day_made": "[2019-09-03 Tue]",
          "name": "1 Pot Chickpea Shakshuka",
          "slug": "shakshuka",
          "is_vegan": "true",
          "is_vegetarian": "true",
          "ease_of_making": "5/5",
          "rating": "4/5"
        },
        "logbook": []
      },
      "ingredients": {
        "keys": [
          "Ingredient",
          "Quantity",
          "Unit"
        ],
        "data": [
          {
            "Ingredient": "Olive Oil",
            "Quantity": "1",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "White onion (or: shallot)",
            "Quantity": "1/2/",
            "Unit": "cups"
          },
          {
            "Ingredient": "Garlic",
            "Quantity": "3",
            "Unit": "cloves"
          },
          {
            "Ingredient": "Bell Pepper (chopped)",
            "Quantity": "1/2/",
            "Unit": "cups"
          },
          {
            "Ingredient": "Diced tomatoes",
            "Quantity": "28",
            "Unit": "oz"
          },
          {
            "Ingredient": "Tomato paste",
            "Quantity": "3",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Maple syrup",
            "Quantity": "1",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Ground cumin",
            "Quantity": "1",
            "Unit": "tsp"
          },
          {
            "Ingredient": "Paprika",
            "Quantity": "1",
            "Unit": "tsp"
          },
          {
            "Ingredient": "Chili powder",
            "Quantity": "2",
            "Unit": "tsp"
          },
          {
            "Ingredient": "Ground cinnamon",
            "Quantity": "1/4",
            "Unit": "tsp"
          },
          {
            "Ingredient": "Chickpeas",
            "Quantity": "24",
            "Unit": "oz"
          }
        ]
      },
      "instructions": [
        {
          "o": "Heat a large metal or cast iron skilled on medium heat.",
          "f": "Heat a large metal or cast iron skilled on medium heat."
        },
        {
          "o": "[t: 00:04:00] When hot, add olive oil, onion, bell pepper and garlic. Saute for 4-5 minutes, stirring frequently.",
          "f": "When hot, add olive oil, onion, bell pepper and garlic. Saute for 4-5 minutes, stirring frequently.",
          "timer": "00:04:00"
        },
        {
          "o": "Add diced tomatoes, tomoato paste, maple syrup, sea salt, paprika, cumin, chili powser, cinnamon. Stir.",
          "f": "Add diced tomatoes, tomoato paste, maple syrup, sea salt, paprika, cumin, chili powser, cinnamon. Stir."
        },
        {
          "o": "[t: 00:03:00] Bring to a simmer. Cook for 2-3 minutes.",
          "f": "Bring to a simmer. Cook for 2-3 minutes.",
          "timer": "00:03:00"
        },
        {
          "o": "[t: 00:15:00] Add chickpeas and olives. Reduce heaet to medium-low for 15-20 minutes.",
          "f": "Add chickpeas and olives. Reduce heaet to medium-low for 15-20 minutes.",
          "timer": "00:15:00"
        }
      ]
    },
    "collard-green-pasta": {
      "meta": {
        "properties": {
          "original_recipe": "https://cookieandkate.com/lemon-collard-greens-pasta/",
          "day_made": "[2019-09-04 Wed]",
          "slug": "collard-green-pasta",
          "is_vegan": "false",
          "is_vegetarian": "true",
          "name": "Leafy Spaghetti",
          "ease_of_making": "5/5",
          "rating": "5/5"
        },
        "logbook": []
      },
      "ingredients": {
        "keys": [
          "Ingredient",
          "Quantity",
          "Unit"
        ],
        "data": [
          {
            "Ingredient": "Collard Greens (or: Swiss Chard)",
            "Quantity": "10",
            "Unit": "leaves"
          },
          {
            "Ingredient": "Spaghetti",
            "Quantity": "1/3",
            "Unit": "package"
          },
          {
            "Ingredient": "Chopped Nuts",
            "Quantity": "3",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Olive Oil",
            "Quantity": "1",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Garlic",
            "Quantity": "2",
            "Unit": "cloves"
          },
          {
            "Ingredient": "Red Pepper Flakes",
            "Quantity": "1",
            "Unit": "pinch"
          },
          {
            "Ingredient": "Parmesan Cheese",
            "Quantity": "1",
            "Unit": "ounce"
          },
          {
            "Ingredient": "Lemon",
            "Quantity": "1",
            "Unit": ""
          }
        ]
      },
      "instructions": [
        {
          "o": "Bring a pot of salted water to boil. Cook the pasta.",
          "f": "Bring a pot of salted water to boil. Cook the pasta."
        },
        {
          "o": "Drain the pasta, reserving a bit of cooking water. Set aside.",
          "f": "Drain the pasta, reserving a bit of cooking water. Set aside."
        },
        {
          "o": "Cut out the \"rib\" of the collard greens. Roll them up like a cigar. Slice across the role, as thinly as possible.",
          "f": "Cut out the \"rib\" of the collard greens. Roll them up like a cigar. Slice across the role, as thinly as possible."
        },
        {
          "o": "Heat a skillet on medium heat. Toast the nuts until fragrant. Set aside.",
          "f": "Heat a skillet on medium heat. Toast the nuts until fragrant. Set aside."
        },
        {
          "o": "Using the same skillet, heat the olive oil. When hot, put in the garlic and red pepper flakes.",
          "f": "Using the same skillet, heat the olive oil. When hot, put in the garlic and red pepper flakes."
        },
        {
          "o": "After a bit, toss in the greens. Sprinkle with salt. Stit often, about three minutes.",
          "f": "After a bit, toss in the greens. Sprinkle with salt. Stit often, about three minutes."
        },
        {
          "o": "Scoop greens into pasta pot and toss with oil. Add pasta water if necessary.",
          "f": "Scoop greens into pasta pot and toss with oil. Add pasta water if necessary."
        },
        {
          "o": "Divide onto plates, top with nuts and parmesan shavings. Add two big lemon wedges per person.",
          "f": "Divide onto plates, top with nuts and parmesan shavings. Add two big lemon wedges per person."
        }
      ]
    },
    "buddha-bowl": {
      "meta": {
        "properties": {
          "original_recipe": "https://tasty.co/recipe/protein-packed-buddha-bowl",
          "day_made": "[2019-09-08]",
          "slug": "buddha-bowl",
          "is_vegan": "true",
          "name": "Protein Bowl",
          "is_vegetarian": "true",
          "ease_of_making": "3/5",
          "rating": "5/5"
        },
        "logbook": []
      },
      "ingredients": {
        "keys": [
          "Ingredient",
          "Quantity",
          "Unit"
        ],
        "data": [
          {
            "Ingredient": "Tofu",
            "Quantity": "8",
            "Unit": "oz"
          },
          {
            "Ingredient": "Sweet Potato",
            "Quantity": "1",
            "Unit": ""
          },
          {
            "Ingredient": "Onion",
            "Quantity": "1",
            "Unit": ""
          },
          {
            "Ingredient": "Garlic",
            "Quantity": "2",
            "Unit": ""
          },
          {
            "Ingredient": "Olive Oil",
            "Quantity": "3",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Chickpeas",
            "Quantity": "1",
            "Unit": "cups"
          },
          {
            "Ingredient": "Salt",
            "Quantity": "1",
            "Unit": "tsp"
          },
          {
            "Ingredient": "Pepper",
            "Quantity": "1/2",
            "Unit": "tsp"
          },
          {
            "Ingredient": "Chili powder",
            "Quantity": "1",
            "Unit": "tsp"
          },
          {
            "Ingredient": "Garlic powder",
            "Quantity": "1",
            "Unit": "tsp"
          },
          {
            "Ingredient": "Quinoa",
            "Quantity": "1+1/2",
            "Unit": "cups"
          },
          {
            "Ingredient": "Shredded Carrots",
            "Quantity": "1/4",
            "Unit": "cups"
          },
          {
            "Ingredient": "Avacado",
            "Quantity": "1",
            "Unit": ""
          },
          {
            "Ingredient": "Lemon",
            "Quantity": "1",
            "Unit": ""
          },
          {
            "Ingredient": "Sesame Oil",
            "Quantity": "1/2",
            "Unit": "tsp"
          },
          {
            "Ingredient": "Hot Sauce",
            "Quantity": "1",
            "Unit": "tsp"
          },
          {
            "Ingredient": "Dried thyme",
            "Quantity": "2",
            "Unit": "tsp"
          },
          {
            "Ingredient": "Paprika",
            "Quantity": "1",
            "Unit": "tsp"
          },
          {
            "Ingredient": "",
            "Quantity": "",
            "Unit": ""
          }
        ]
      },
      "instructions": [
        {
          "o": "Make the marinade: combine Olive oil, seasami oil, hot sauce, thyme, paprika, and salt. Set aside.",
          "f": "Make the marinade: combine Olive oil, seasami oil, hot sauce, thyme, paprika, and salt. Set aside."
        },
        {
          "o": "[t: 00:30:00] Add marinade and tofu to a container and marinate for 30 minutes, up to a day",
          "f": "Add marinade and tofu to a container and marinate for 30 minutes, up to a day",
          "timer": "00:30:00"
        },
        {
          "o": "Preheat the oven to 400ºF (200ºC).",
          "f": "Preheat the oven to 400ºF (200ºC)."
        },
        {
          "o": "[t: 00:20:00] Cut sweet potato into cubes. Slice the onion, dice garlic. Put it all on a baking sheet. Drizzel with oil, season with salt and pepper. Bake for 20-25 min.",
          "f": "Cut sweet potato into cubes. Slice the onion, dice garlic. Put it all on a baking sheet. Drizzel with oil, season with salt and pepper. Bake for 20-25 min.",
          "timer": "00:20:00"
        },
        {
          "o": "In a medium bowl, add the chickpeas, salt, pepper, chili powder, and garlic powder. Stir to combine.",
          "f": "In a medium bowl, add the chickpeas, salt, pepper, chili powder, and garlic powder. Stir to combine."
        },
        {
          "o": "[t: 00:10:00] Transfer chickpeas to skilled and cook on medium heat for 10 minutes. Set aside.",
          "f": "Transfer chickpeas to skilled and cook on medium heat for 10 minutes. Set aside.",
          "timer": "00:10:00"
        },
        {
          "o": "[t: 00:10:00] Fry the tofu in the same pan for about 10 minutes on each side.",
          "f": "Fry the tofu in the same pan for about 10 minutes on each side.",
          "timer": "00:10:00"
        },
        {
          "o": "Slice tofu as you like.",
          "f": "Slice tofu as you like."
        },
        {
          "o": "Combine with quinoa, greens, sweet potatoes, onions, chickpeas, carrots, tofu, and avocado.",
          "f": "Combine with quinoa, greens, sweet potatoes, onions, chickpeas, carrots, tofu, and avocado."
        }
      ]
    },
    "massaman-curry": {
      "meta": {
        "properties": {
          "slug": "massaman-curry",
          "original_recipe": "https://minimalistbaker.com/easy-1-pot-massaman-curry/",
          "day_made": "[2019-09-10]",
          "is_vegan": "true",
          "name": "Massaman Curry",
          "is_vegetarian": "true",
          "ease_of_making": "5/5",
          "rating": "5/5"
        },
        "logbook": []
      },
      "ingredients": {
        "keys": [
          "Ingredient",
          "Quantity",
          "Unit"
        ],
        "data": [
          {
            "Ingredient": "Tofu",
            "Quantity": "1",
            "Unit": "package"
          },
          {
            "Ingredient": "Coconut oil",
            "Quantity": "2",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Cumin powder",
            "Quantity": "1",
            "Unit": "tsp"
          },
          {
            "Ingredient": "Coriander powder",
            "Quantity": "1",
            "Unit": "tsp"
          },
          {
            "Ingredient": "Red Curry Paste",
            "Quantity": "5",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Baby potatoes",
            "Quantity": "1+1/2",
            "Unit": "cups"
          },
          {
            "Ingredient": "Carrots",
            "Quantity": "2",
            "Unit": ""
          },
          {
            "Ingredient": "Coconut milk",
            "Quantity": "28",
            "Unit": "oz"
          },
          {
            "Ingredient": "Water",
            "Quantity": "1+1/2",
            "Unit": "cups"
          },
          {
            "Ingredient": "Ground cinnamon",
            "Quantity": "1/4",
            "Unit": "tsp"
          },
          {
            "Ingredient": "Soy sauce",
            "Quantity": "2",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Maple syrup",
            "Quantity": "2",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Peanut butter",
            "Quantity": "2",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Lime juice",
            "Quantity": "2",
            "Unit": "tbsp"
          }
        ]
      },
      "instructions": [
        {
          "o": "Press your tofu. You'll come back and cube it once pressed.",
          "f": "Press your tofu. You'll come back and cube it once pressed."
        },
        {
          "o": "Heat a large pot or dutch oven on medium heat. Once hot, add oil and shallot.",
          "f": "Heat a large pot or dutch oven on medium heat. Once hot, add oil and shallot."
        },
        {
          "o": "[t: 00:02:00] Saute for 2 minutes. Stir frequently.",
          "f": "Saute for 2 minutes. Stir frequently.",
          "timer": "00:02:00"
        },
        {
          "o": "[t: 00:02:00] Add cumin and coriander. Saute for 2 minutes, stirring frequently.",
          "f": "Add cumin and coriander. Saute for 2 minutes, stirring frequently.",
          "timer": "00:02:00"
        },
        {
          "o": "[t: 00:01:00] Add red currey paste and stir for another minute.",
          "f": "Add red currey paste and stir for another minute.",
          "timer": "00:01:00"
        },
        {
          "o": "[t: 00:02:00] Add potatoes and carrots and stire to coat. Cook for 2 minutes.",
          "f": "Add potatoes and carrots and stire to coat. Cook for 2 minutes.",
          "timer": "00:02:00"
        },
        {
          "o": "Add coconut milk, water, cinnamon, soysauce, maple syrup, and peanut butter. Add your tofu or other protein now.",
          "f": "Add coconut milk, water, cinnamon, soysauce, maple syrup, and peanut butter. Add your tofu or other protein now."
        },
        {
          "o": "[t: 00:10:00] Bring to a simmer and cook for 10-15 minutes uncovered. Don't boil it. Simmer it.",
          "f": "Bring to a simmer and cook for 10-15 minutes uncovered. Don't boil it. Simmer it.",
          "timer": "00:10:00"
        },
        {
          "o": "Near the end of the previous step, toss in your tofu and the lime. Cook for another few minutes.",
          "f": "Near the end of the previous step, toss in your tofu and the lime. Cook for another few minutes."
        },
        {
          "o": "[t: 00:05:00] Let stand for 10 minutes.",
          "f": "Let stand for 10 minutes.",
          "timer": "00:05:00"
        },
        {
          "o": "Enjoy with a side of rice.",
          "f": "Enjoy with a side of rice."
        }
      ]
    },
    "cauliflower-rice-stir-fry": {
      "meta": {
        "properties": {
          "slug": "cauliflower-rice-stir-fry",
          "original_recipe": "https://minimalistbaker.com/30-minute-cauliflower-rice-stir-fry/",
          "day_made": "[2019-09-12]",
          "is_vegan": "true",
          "is_vegetarian": "true",
          "name": "Cauliflower Rice Stir Fry",
          "ease_of_making": "5/5",
          "rating": "5/5"
        },
        "logbook": []
      },
      "ingredients": {
        "keys": [
          "Ingredient",
          "Quantity",
          "Unit"
        ],
        "data": [
          {
            "Ingredient": "Cauliflower",
            "Quantity": "1",
            "Unit": "head"
          },
          {
            "Ingredient": "Water (for cauliflower rice)",
            "Quantity": "3",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Coconut oil",
            "Quantity": "2",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Peanut or Almond Butter",
            "Quantity": "2",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "ginger",
            "Quantity": "1",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Maple syrup",
            "Quantity": "1",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Soy sauce (the sauce)",
            "Quantity": "4",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Soy sauce (the stir fry)",
            "Quantity": "3",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Lime juice",
            "Quantity": "2",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Chili garlic sauce or sriracha",
            "Quantity": "3",
            "Unit": "tsp"
          },
          {
            "Ingredient": "Garlic",
            "Quantity": "2-4",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Water",
            "Quantity": "-",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Green beans (trimmed and halved)",
            "Quantity": "1+1/2",
            "Unit": "cups"
          },
          {
            "Ingredient": "Cabbage (thinly sliced)",
            "Quantity": "1",
            "Unit": "cups"
          },
          {
            "Ingredient": "Green onions (diced)",
            "Quantity": "1",
            "Unit": "cups"
          },
          {
            "Ingredient": "Bell Pepper (thin slices, lengthwise)",
            "Quantity": "1",
            "Unit": "-"
          },
          {
            "Ingredient": "Cashews",
            "Quantity": "3/4",
            "Unit": "cups"
          },
          {
            "Ingredient": "A Blender or a grater for the cauliflower",
            "Quantity": "",
            "Unit": ""
          }
        ]
      },
      "instructions": [
        {
          "o": "Wash the cauliflower. Chop it in a blender until you have small chunks (that look more like rice than cauliflower.)",
          "f": "Wash the cauliflower. Chop it in a blender until you have small chunks (that look more like rice than cauliflower.)"
        },
        {
          "o": "Create the sauce: add coconut oil, peanut butter, soy sauce lime juice, chili garlic sauce, fresh ginger, maple syrup, and water into a small bowl. Whisk to combine. Set aside.",
          "f": "Create the sauce: add coconut oil, peanut butter, soy sauce lime juice, chili garlic sauce, fresh ginger, maple syrup, and water into a small bowl. Whisk to combine. Set aside."
        },
        {
          "o": "[t: 00:05:00] Heat a skillet over medium-low. Add cauliflower rice and water. Stir and cover with a lid to steam. Cook for 4-6 minutes or until just tender. Remove and let some steam out.",
          "f": "Heat a skillet over medium-low. Add cauliflower rice and water. Stir and cover with a lid to steam. Cook for 4-6 minutes or until just tender. Remove and let some steam out.",
          "timer": "00:05:00"
        },
        {
          "o": "[t: 00:04:00] Heat another skillet (or pot) on medium heat. When hot, add coconut oil and green beans. Season with 1/3 of the soy sauce used for the stirfry (1tbsp). Cover with lid and steam for about 4 minutes.",
          "f": "Heat another skillet (or pot) on medium heat. When hot, add coconut oil and green beans. Season with 1/3 of the soy sauce used for the stirfry (1tbsp). Cover with lid and steam for about 4 minutes.",
          "timer": "00:04:00"
        },
        {
          "o": "[t: 00:03:00] Add bell peppers, green onion, cabbage, and remaining soy sauce (2tbsp). Stir. Saute for 3-4 minutes.",
          "f": "Add bell peppers, green onion, cabbage, and remaining soy sauce (2tbsp). Stir. Saute for 3-4 minutes.",
          "timer": "00:03:00"
        },
        {
          "o": "Add cashes and cauliflower rice and stire to combine.",
          "f": "Add cashes and cauliflower rice and stire to combine."
        },
        {
          "o": "[t: 00:03:00] Add the sauce to the beans. Increase heat to medium-high. Cook for about 3 minutes until hot.",
          "f": "Add the sauce to the beans. Increase heat to medium-high. Cook for about 3 minutes until hot.",
          "timer": "00:03:00"
        }
      ]
    },
    "kale-coconut-rice": {
      "meta": {
        "properties": {
          "slug": "kale-coconut-rice",
          "original_recipe": "https://cookieandkate.com/spicy-kale-and-coconut-fried-rice/",
          "time": "00:40:00",
          "day_made": "[2019-09-17]",
          "is_vegan": "false",
          "name": "Kale And Coconut Rice",
          "is_vegetarian": "true",
          "ease_of_making": "4",
          "rating": "5"
        },
        "logbook": []
      },
      "ingredients": {
        "keys": [
          "Ingredient",
          "Quantity",
          "Unit"
        ],
        "data": [
          {
            "Ingredient": "coconut oil",
            "Quantity": "2",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "eggs",
            "Quantity": "2",
            "Unit": "-"
          },
          {
            "Ingredient": "garlic (minced)",
            "Quantity": "2",
            "Unit": ""
          },
          {
            "Ingredient": "green onions (chopped)",
            "Quantity": "1",
            "Unit": "bunch"
          },
          {
            "Ingredient": "Chopped vegetables (peppers, brussel sprouts etc)",
            "Quantity": "1",
            "Unit": "cups"
          },
          {
            "Ingredient": "Kale (ribs removed, chopped)",
            "Quantity": "1",
            "Unit": "bunch"
          },
          {
            "Ingredient": "Sea salt",
            "Quantity": "1/4",
            "Unit": "tsp"
          },
          {
            "Ingredient": "unsweetened coconut flakes or shreds",
            "Quantity": "3/4",
            "Unit": "cups"
          },
          {
            "Ingredient": "Rice (uncooked)",
            "Quantity": "1",
            "Unit": "cup"
          },
          {
            "Ingredient": "Soy sauce",
            "Quantity": "2",
            "Unit": "tsp"
          },
          {
            "Ingredient": "Sriracha",
            "Quantity": "2",
            "Unit": "tsp"
          },
          {
            "Ingredient": "Lime (halved)",
            "Quantity": "1",
            "Unit": ""
          },
          {
            "Ingredient": "Cilantro",
            "Quantity": "",
            "Unit": ""
          }
        ]
      },
      "instructions": [
        {
          "o": "Cook the rice and set it aside to cool.",
          "f": "Cook the rice and set it aside to cool."
        },
        {
          "o": "Heat a large skillet on medium high heat. Add 1 teaspoon of coconut oil.",
          "f": "Heat a large skillet on medium high heat. Add 1 teaspoon of coconut oil."
        },
        {
          "o": "Add eggs and stir frequently so they are sort of scrambled. Transfer to bowl when done and wipe the pan clean.",
          "f": "Add eggs and stir frequently so they are sort of scrambled. Transfer to bowl when done and wipe the pan clean."
        },
        {
          "o": "Add a tablespoon of coconut oil to pan. Add garlic, green onions and vegetables. Cook until fragrant (30 seconds).  Add the kale and salt . Cook until wilted, about 1-2 minutes. Transfer to bowl with eggs.",
          "f": "Add a tablespoon of coconut oil to pan. Add garlic, green onions and vegetables. Cook until fragrant (30 seconds).  Add the kale and salt . Cook until wilted, about 1-2 minutes. Transfer to bowl with eggs."
        },
        {
          "o": "Add 2 more tablespoons of coconut oil to the pan AGAIN. Add the coconut flakes, stir frequently until golden. Add the cooked rice and stir occaisionally until hot, about 3 minutes",
          "f": "Add 2 more tablespoons of coconut oil to the pan AGAIN. Add the coconut flakes, stir frequently until golden. Add the cooked rice and stir occaisionally until hot, about 3 minutes"
        },
        {
          "o": "Pour the contents of the bowl back into the pan, breaking up the scrambled egg with a spatula.",
          "f": "Pour the contents of the bowl back into the pan, breaking up the scrambled egg with a spatula."
        },
        {
          "o": "Add tamari, Sriracha, and half of the lime juice. Stir.",
          "f": "Add tamari, Sriracha, and half of the lime juice. Stir."
        }
      ]
    },
    "pear-brie-salad": {
      "meta": {
        "properties": {
          "slug": "pear-brie-salad",
          "original_recipe": "https://www.theorganickitchen.org/pears-and-brie-salad-with-honey-champagne-vinaigrette/",
          "time": "00:40:00",
          "day_made": "[2019-09-19]",
          "is_vegan": "false",
          "is_vegetarian": "true",
          "name": "Pear + Brie Salad",
          "ease_of_making": "5/5",
          "rating": "5/5"
        },
        "logbook": []
      },
      "ingredients": {
        "keys": [
          "Ingredient",
          "Quantity",
          "Unit"
        ],
        "data": [
          {
            "Ingredient": "Greens: Spinach / Arugla etc",
            "Quantity": "4",
            "Unit": "cups"
          },
          {
            "Ingredient": "Broccoli",
            "Quantity": "2",
            "Unit": "cups"
          },
          {
            "Ingredient": "Potatoes (chopped)",
            "Quantity": "1",
            "Unit": "cups"
          },
          {
            "Ingredient": "Dill",
            "Quantity": "1",
            "Unit": "sprig"
          },
          {
            "Ingredient": "Pears (sliced)",
            "Quantity": "2",
            "Unit": "--"
          },
          {
            "Ingredient": "Brie Cheeze",
            "Quantity": "4-6",
            "Unit": "slices"
          },
          {
            "Ingredient": "Cranberries",
            "Quantity": "1/3",
            "Unit": "cups"
          },
          {
            "Ingredient": "Avocado",
            "Quantity": "1/2",
            "Unit": "--"
          },
          {
            "Ingredient": "Pecans (chopped)",
            "Quantity": "1/3",
            "Unit": "cups"
          },
          {
            "Ingredient": "Olive oil",
            "Quantity": "1/2",
            "Unit": "cups"
          },
          {
            "Ingredient": "Apple cider vinegar",
            "Quantity": "2",
            "Unit": "tsp"
          },
          {
            "Ingredient": "Rice wine vinegar",
            "Quantity": "1/4",
            "Unit": "cups"
          },
          {
            "Ingredient": "Shallots (minced)",
            "Quantity": "1",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Maple Syrup",
            "Quantity": "1",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Pepper",
            "Quantity": "1",
            "Unit": "pinch"
          }
        ]
      },
      "instructions": [
        {
          "o": "[t: 00:12:00] Preheat oven to 425. Add chopped potatoes and broccoli florets to bowl with olive oil and salt and pepper and dill. Put on a baking sheet on parchment paper. Bake for 10-12 minutes",
          "f": "Preheat oven to 425. Add chopped potatoes and broccoli florets to bowl with olive oil and salt and pepper and dill. Put on a baking sheet on parchment paper. Bake for 10-12 minutes",
          "timer": "00:12:00"
        },
        {
          "o": "Prepare the dressing for the salad. Add the following to a container and whisk: Olive oil, Rice wine vinegar, shallots, maple syrup, ground pepper.",
          "f": "Prepare the dressing for the salad. Add the following to a container and whisk: Olive oil, Rice wine vinegar, shallots, maple syrup, ground pepper."
        },
        {
          "o": "Prepare the salad: Lightly apply the dressing to the greens Arrange pear slices and briece on plate.  Drizzle with dressing. Add greens, cranberries, pecans, and extra slices of pear. Add a bit more dressing.",
          "f": "Prepare the salad: Lightly apply the dressing to the greens Arrange pear slices and briece on plate.  Drizzle with dressing. Add greens, cranberries, pecans, and extra slices of pear. Add a bit more dressing."
        },
        {
          "o": "Add the potatoes and broccoli as a side or mixed with the salad.",
          "f": "Add the potatoes and broccoli as a side or mixed with the salad."
        }
      ]
    },
    "sweet-potato-gnocci": {
      "meta": {
        "properties": {
          "slug": "sweet-potato-gnocci",
          "original_recipe": "https://biancazapatka.com/en/vegan-sweet-potato-gnocchi/",
          "time": "00:40:00",
          "day_made": "[2019-09-22]",
          "is_vegan": "false",
          "is_vegetarian": "true",
          "name": "Sweet Potato Gnocchi",
          "ease_of_making": "-/5",
          "rating": "_/5"
        },
        "logbook": []
      },
      "ingredients": {
        "keys": [
          "Ingredient",
          "Quantity",
          "Unit"
        ],
        "data": [
          {
            "Ingredient": "Sweet Potato (large / 600g)",
            "Quantity": "1",
            "Unit": "--"
          },
          {
            "Ingredient": "Nutritional Yeast or Parmesan (optional)",
            "Quantity": "2-3",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Flour",
            "Quantity": "1+1/3 (or is it 3/4 cups?)",
            "Unit": "cups"
          },
          {
            "Ingredient": "Salt",
            "Quantity": "1/2",
            "Unit": "tsp"
          },
          {
            "Ingredient": "Vegan butter or coconut oil",
            "Quantity": "2",
            "Unit": "tbsp"
          },
          {
            "Ingredient": "Garlic",
            "Quantity": "3",
            "Unit": "--"
          },
          {
            "Ingredient": "Optional Nuts (pine or pecan, or whatever)",
            "Quantity": "4",
            "Unit": "tbsp"
          }
        ]
      },
      "instructions": [
        {
          "o": "[t: 00:50:00] Prick the sweet potato several times with a fork. Put it in the oven for 50-60 minutes at about 425 degrees.",
          "f": "Prick the sweet potato several times with a fork. Put it in the oven for 50-60 minutes at about 425 degrees.",
          "timer": "00:50:00"
        },
        {
          "o": "Peel the sweet potato. Scoop the inside into a bowl. Mash it smooth. Add salt. Add nutritional yeast if you want. Add the flour and knead gently. Don't over knead. Try to use as little flour as possible.",
          "f": "Peel the sweet potato. Scoop the inside into a bowl. Mash it smooth. Add salt. Add nutritional yeast if you want. Add the flour and knead gently. Don't over knead. Try to use as little flour as possible."
        },
        {
          "o": "Transfer the dough to a floured work surface. Form it in a flat ball and cut into quarters. Take a piece and form a long rope, rolling it into a long rope. Cut the rope into 2cm pieces.",
          "f": "Transfer the dough to a floured work surface. Form it in a flat ball and cut into quarters. Take a piece and form a long rope, rolling it into a long rope. Cut the rope into 2cm pieces."
        },
        {
          "o": "Create gnocchi \"ridges\" into the sweet potato pieces by rolling it along the tines of a fork (from the tip backward).",
          "f": "Create gnocchi \"ridges\" into the sweet potato pieces by rolling it along the tines of a fork (from the tip backward)."
        },
        {
          "o": "Bring a pot of salted water to a boil. Add the gnocchi, cook until they float to the top of the water. Drain. Optional: toss with a little olive oil to prevent sticking.",
          "f": "Bring a pot of salted water to a boil. Add the gnocchi, cook until they float to the top of the water. Drain. Optional: toss with a little olive oil to prevent sticking."
        },
        {
          "o": "If you have more gnocchi than you need, now is the time to freeze the abundance.",
          "f": "If you have more gnocchi than you need, now is the time to freeze the abundance."
        },
        {
          "o": "[t: 00:15:00] Put tomatoes onto a baking sheet with cloves of garlic. Drizzle  with olive oil. Season with salt and pepper. Roast in the oven at 400 for 15 minutes.",
          "f": "Put tomatoes onto a baking sheet with cloves of garlic. Drizzle  with olive oil. Season with salt and pepper. Roast in the oven at 400 for 15 minutes.",
          "timer": "00:15:00"
        },
        {
          "o": "Toast nuts in a small pan with additional oil. Set aside.",
          "f": "Toast nuts in a small pan with additional oil. Set aside."
        },
        {
          "o": "Heat butter or coconut oil on medium. Add the gnocchi and roast until golden-brown and crispy.",
          "f": "Heat butter or coconut oil on medium. Add the gnocchi and roast until golden-brown and crispy."
        },
        {
          "o": "Serve Gnocchi with roasted tomatoes and nuts and sprinkle with cheese or fresh chooped herbs if you like.",
          "f": "Serve Gnocchi with roasted tomatoes and nuts and sprinkle with cheese or fresh chooped herbs if you like."
        }
      ]
    }
  }
}