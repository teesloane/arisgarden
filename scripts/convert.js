...
function getInstructions(n) {
  ...
  let out = []
  list.forEach(l => l.children.map(j => {
    ...
    switch (matchCategory) {
      ...
      case "#":
        out["f"] = ogstr.replace(ogRE, (match) => {
          let tagContent = match.split("|")[1].trim().slice(0, -1);
          let tagId = match.split("|")[0].slice(3).trim()
...
function tableParser(tableChildren) {
  return tableChildren.reduce((acc, curr, idx) => {
